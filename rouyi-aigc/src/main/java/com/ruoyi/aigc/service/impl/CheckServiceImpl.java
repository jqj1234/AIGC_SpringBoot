...
import com.ruoyi.aigc.vo.respvo.AnalysisResult;
...
//    private String url = "http://47.109.58.42:4010";
     ...
     */
//    @Override
//    public AjaxResult checkText(MultipartFile file,String value) throws IOException {
////        String data = "{\n" +
...
//    }
    ...
    @Override
    public AjaxResult checkText(HashMap<String,String> info){
//        String data = "{\n" +
//                "        \"whole_ai_prob\": 48.103,\n" +
//                "        \"whole_real_prob\": 51.897,\n" +
//                "        \"details\": [\n" +
...
//        // 设置请求体，包括文件和其他参数
        ...
        body.add("url", info.get("url"));
        body.add("model",info.get("model"));
////
////        // 创建请求实体对象
...
//         创建 ObjectMapper 实例
        ...
        try {
//
            analysisResult = mapper.readValue(json, AnalysisResult.class);
        }catch (JsonProcessingException jsonProcessingException){
            return AjaxResult.error("数据解析失败");
        }
        String wholeAiProb = String.valueOf(analysisResult.getWhole_ai_prob()) + "%";
        String wholeRealProb = String.valueOf(analysisResult.getWhole_real_prob()) + "%";
        LoginUser loginUser = getLoginUser();
        Long userId = loginUser.getUserId();
        UserFileRecord userFileRecord = new UserFileRecord();
        userFileRecord.setId(idWorker.nextId());
        userFileRecord.setDatetime(DateTime.now().toDate());
        userFileRecord.setUserId(userId);
        userFileRecord.setUsername(loginUser.getUsername());
        userFileRecord.setFileName(info.get("originalFilename"));
        userFileRecord.setUrl(info.get("url"));
        userFileRecord.setWholeAiProb(wholeAiProb);
        userFileRecord.setType(Integer.valueOf(info.get("model")));
        userFileRecord.setWholeRealProb(wholeRealProb);
     ...
     */
    @Override
    public AjaxResult checkImage(HashMap<String,String> info){
...
//         创建 ObjectMapper 实例
        ObjectMapper mapper = new ObjectMapper();
        HashMap<String,String> analysisResult = null;
        try {
            analysisResult = mapper.readValue(json, HashMap.class);
        } catch (JsonProcessingException e) {
            return AjaxResult.error("图片解析错误");
        }
        String aiProb = analysisResult.get("ai_prob")+"%";
        LoginUser loginUser = getLoginUser();
        Long userId = loginUser.getUserId();

        UserImageRecord userImageRecord = new UserImageRecord();
        userImageRecord.setId(idWorker.nextId());
        userImageRecord.setUserId(userId);
        userImageRecord.setUsername(loginUser.getUsername());
        userImageRecord.setType(info.get("type"));
        userImageRecord.setUrl(info.get("url"));
        userImageRecord.setAiProb(aiProb);
        userImageRecord.setDatetime(DateTime.now().toDate());
        userImageRecordMapper.insert(userImageRecord);
     ...
     */
    @Override
    public AjaxResult photoOcr(MultipartFile file) {
        return null;
    }
}
